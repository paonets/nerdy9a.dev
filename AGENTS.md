...
- **Customized Plugins Strategy:** All customized Quartz plugins (such as `content-page`, `folder-page`, `content-meta`, `note-properties`) MUST be stored in the `custom-plugins/` directory and referenced in `quartz.config.yaml` using local paths (e.g. `source: ./custom-plugins/plugin-name`). Do NOT edit files inside `.quartz/plugins/` directly, as `.quartz/` is gitignored and your changes will be lost upon deployment. Whenever you modify or create a custom plugin, you MUST document the changes in the master index: [CUSTOMIZATIONS.md](file:///Users/pongsakorn/Projects/nerdy9a.dev/CUSTOMIZATIONS.md).
