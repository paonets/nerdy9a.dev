  ...
  try {
    ...
    for (const filePath of targetFiles) {
      ...
      if ((ext === ".html" || ext === ".htm") && typeof det.detectHtml === "function") {
        try {
          findings = await det.detectHtml(filePath)
        } catch {
          findings = []
          detectorThrew = true
        }
        ...
        try {
          findings = await det.detectText(content, filePath)
        } catch {
          findings = []
          detectorThrew = true
        }
    ...
    if (pendingWinner && shouldEmitAckForFile(pendingWinner.filePath)) {
      const text = renderPendingAck(pendingWinner.filePath, pendingWinner.known, {
        cwd: projectCwd,
      })
      return {
        exitCode: 0,
