...
function writeConfig(cwd, hookConfig, opts = {}) {
  ...
  const existingHook =
    existing.hook && typeof existing.hook === "object" && !Array.isArray(existing.hook)
      ? existing.hook
      : {}
...
function mergeConfig(existing) {
  ...
  return {
    ...
    ignoreRules: Array.isArray(base.ignoreRules)
      ? Array.from(new Set(base.ignoreRules.map(String)))
      : [],
    ignoreFiles: Array.isArray(base.ignoreFiles)
      ? Array.from(new Set(base.ignoreFiles.map(String)))
      : [],
