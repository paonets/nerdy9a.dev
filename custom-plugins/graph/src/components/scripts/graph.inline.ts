...
(function () {
  ...
  function checkAndLoad() {
    ...
    var isVisible = false;
    for (var i = 0; i < containers.length; i++) {
      if (containers[i].offsetWidth > 0 || containers[i].offsetHeight > 0) {
        isVisible = true;
        break;
      }
    }
  ...
  // Initial check
  var loadedOnStart = checkAndLoad();

  // If not loaded on start, set up listeners to load it when needed
  if (!loadedOnStart) {
    // 1. Listen for global graph icon clicks
    var setupClickListeners = function () {
      var globalGraphIcons = document.querySelectorAll(".global-graph-icon");
      var loadAndShow = function () {
        globalGraphIcons.forEach(function (btn) {
          btn.removeEventListener("click", loadAndShow);
        });
        loadLibraries();
      };
      globalGraphIcons.forEach(function (btn) {
    ...
    setupClickListeners();
    document.addEventListener("nav", setupClickListeners);

    // 2. Listen for navigation/rendering (in case they navigate to a page with a visible graph)
    document.addEventListener("nav", checkAndLoad);
    document.addEventListener("render", checkAndLoad);
